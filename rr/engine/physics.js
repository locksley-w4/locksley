...
function jumpEvent(e) {
    if (e.key === " " && parseInt(player.getBoundingClientRect().bottom) >= parseInt(groundBounding.top - 3)) {
        jump(player);
        jumpingAnim()
    }
}
function jumpEventMobile(e) {
    if (parseInt(player.getBoundingClientRect().bottom) >= parseInt(groundBounding.top - 3)) {
        // console.log(e);
        jump(player);
        jumpingAnim()
    }
