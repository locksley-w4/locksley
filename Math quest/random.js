...
function randNumber(min, max, maxnad = 4) {
    let isInt = torf();

    if (isInt) return Math.floor(min + Math.random() * (max + 1 - min));
    else return parseFloat((min + Math.random() * (max + 1 - min)).toFixed(rand(1, maxnad)));
}
